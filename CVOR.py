#!/usr/bin/env python3
"""
VOR / ASRACS / SAAF Airport Monitoring System v7.0
Merged from v5.3 (CVOR1) and v6.0 (CVOR2).
Supports full desktop GUI (PyQt5 + OpenGL) and headless --self-test mode.
"""
...
SYSTEM_OPERATIONS_REFERENCE = (
    ...
    "1. VOR monitoring and signal-quality supervision for civil and SAAF stations.\n"
    "2. Approach guidance support including glide-slope trend awareness and terrain context.\n"
    "3. ASRACS-style surface surveillance for runway, taxiway, apron, and hotspot activity.\n"
    "4. SAAF operational overview with base metadata, communications frequencies, and readiness context.\n"
    "5. Connection resilience across synthetic, TCP, serial, and archived LDA data sources.\n"
    "\n"
    "Recommended operating flow:\n"
    "- Select the monitored station or airfield from the grouped station database.\n"
    "- Confirm ident, bearing, deviation, signal strength, and trend consistency.\n"
    "- Use the radar page for airborne traffic awareness and historical track movement.\n"
    "- Use the approach page for glide-path, localizer, and terrain-awareness checks.\n"
    "- Use the ASRACS page for hotspot, strip-incursion, separation, and conflict alerts.\n"
    "- Review the diagnostics page for acquisition state, tracker counts, and system health.\n"
    "\n"
    "Operational cautions:\n"
    "- Synthetic displays support training, validation, and fallback operation only.\n"
    "- Surface alerts are advisory and must be cross-checked with authoritative tower procedures.\n"
    "- Terrain, VOR, runway, and communications metadata should be validated against current publications.\n"
    "- Loss of external data source should automatically degrade to safe simulated monitoring behaviour.\n"
)
...
DEFAULT_SAAF_BASES: Dict[str, Dict[str, Any]] = {
    "FAWK": {"squadrons": ["21 Sqn (C-130)", "28 Sqn (Transport)"], "twr_freq": 118.1, "app_freq": 119.1, "atis_freq": 127.65, "gnd_freq": 121.7, "description": "Primary SAAF strategic base near Pretoria"},
    "FALM": {"squadrons": ["2 Sqn (Gripen)", "85 CFS (Hawk)"], "twr_freq": 118.3, "app_freq": 120.1, "atis_freq": 126.2, "gnd_freq": 121.9, "description": "Premier fighter base supporting Gripen and Hawk operations"},
    "FAHS": {"squadrons": ["17 Sqn (Rooivalk)", "19 Sqn (Oryx)"], "twr_freq": 118.5, "app_freq": 119.8, "atis_freq": 125.3, "gnd_freq": 121.8, "description": "Lowveld operational base supporting helicopter and joint-force missions"},
    "FALW": {"squadrons": ["41 Sqn (PC-7 Mk II)", "Flight Test Support"], "twr_freq": 118.7, "app_freq": 119.7, "atis_freq": 126.0, "gnd_freq": 121.6, "description": "Primary SAAF fixed-wing training base on the West Coast"},
    "FAOB": {"squadrons": ["TFDC", "UAV Test Flights"], "twr_freq": 118.9, "app_freq": 120.3, "atis_freq": 125.8, "gnd_freq": 121.75, "description": "Weapons, test and evaluation base with restricted-airspace support"},
    "FASK": {"squadrons": ["SAAF Museum", "Heritage Flights"], "twr_freq": 118.1, "app_freq": 119.1, "atis_freq": 127.65, "gnd_freq": 121.7, "description": "Historic SAAF base and museum field"},
    "FABL": {"squadrons": ["16 Sqn Det", "28 Sqn Det"], "twr_freq": 118.1, "app_freq": 119.6, "atis_freq": 126.8, "gnd_freq": 121.7, "description": "Central South African support base"},
    "FAYP": {"squadrons": ["22 Sqn (Oryx)", "35 Sqn Det"], "twr_freq": 119.0, "app_freq": 120.2, "atis_freq": 127.3, "gnd_freq": 121.9, "description": "Western Cape support base"},
    "FADN": {"squadrons": ["35 Sqn Det", "Maritime Liaison"], "twr_freq": 118.6, "app_freq": 120.6, "atis_freq": 127.0, "gnd_freq": 121.8, "description": "KwaZulu-Natal support base"},
    "FAPE": {"squadrons": ["Liaison Flight", "Reserve Support"], "twr_freq": 118.1, "app_freq": 120.0, "atis_freq": 127.4, "gnd_freq": 121.9, "description": "Eastern Cape support station"},
}
...
def _build_default_saaf_bases() -> Dict[str, Dict[str, Any]]:
    """Return a full CVOR1-style SAAF compatibility dictionary."""
    defaults: Dict[str, Dict[str, Any]] = {}
    for code, station in SAAF_BASES.items():
        entry = copy.deepcopy(station)
        entry["runway"] = _primary_runway_name(station)
        entry["ils_rwy"] = station["ils_runways"][0]["runway"] if station.get("ils_runways") else ""
        entry["airport"] = station["name"]
        defaults[code] = entry
    return defaults
...
def _primary_runway_name(station: Dict[str, Any]) -> str:
    """Return a compact runway string for compatibility dictionaries."""
    if station.get("ils_runways"):
        names = [str(item.get("runway", "")) for item in station["ils_runways"] if item.get("runway")]
        if len(names) >= 2:
            return f"{names[0]}/{names[1]}"
        if names:
            return names[0]
    layout = AIRFIELD_LAYOUTS.get(station.get("icao", ""), {})
    names = [str(item.get("name", "")) for item in layout.get("runways", []) if item.get("name")]
    return names[0] if names else ""


def _build_default_airports() -> Dict[str, Dict[str, Any]]:
    """Build a CVOR1-style airport compatibility dictionary."""
    defaults: Dict[str, Dict[str, Any]] = {}
    for code in ("JNB", "CPT", "DUR"):
        station = copy.deepcopy(VOR_STATIONS[code])
        station["runway"] = _primary_runway_name(station)
        station["airport"] = station["name"]
        defaults[code] = station
    return defaults
...
AIRFIELD_LAYOUTS: Dict[str, Dict[str, Any]] = {
    ...
    "CPT": _layout(
        "Cape Town International",
        [{"name": "01/19", "start": (210, 30), "end": (230, 390), "width": 18}],
        [[(120, 110), (170, 150), (210, 190)], [(260, 120), (240, 170), (225, 220)], [(115, 320), (160, 285), (205, 245)]],
        [(70, 85, 90, 70), (255, 90, 75, 65), (85, 290, 90, 60)],
        [("G1", 92, 112), ("G2", 122, 128), ("CARGO", 280, 112), ("APR", 110, 315)],
        [("HS1", 208, 182, "Primary taxi crossing"), ("HS2", 228, 248, "Departure queue")],
        (0, 0, 400, 420),
    ),
    "DUR": _layout(
        "King Shaka International",
        [{"name": "06/24", "start": (40, 260), "end": (380, 120), "width": 20}],
        [[(95, 305), (150, 275), (215, 240)], [(95, 165), (155, 192), (225, 220)], [(255, 270), (305, 228), (350, 190)]],
        [(55, 285, 92, 65), (60, 145, 88, 55), (265, 255, 78, 60)],
        [("A1", 78, 306), ("A2", 110, 320), ("T1", 102, 165), ("C1", 287, 278)],
        [("HS1", 188, 248, "Terminal crossing"), ("HS2", 258, 214, "Rapid-exit merge")],
        (0, 0, 420, 360),
    ),
    "FAWK": _layout(
        "AFB Waterkloof",
...
DEFAULT_AIRPORTS: Dict[str, Dict[str, Any]] = _build_default_airports()


def _build_default_vor_stations() -> Dict[str, Dict[str, Any]]:
    """Build a CVOR1-style VOR dictionary keyed as ``JNB_VOR`` etc."""
    defaults: Dict[str, Dict[str, Any]] = {}
    for code, station in VOR_STATIONS.items():
        entry = copy.deepcopy(station)
        entry["frequency"] = entry.get("vor_freq")
        entry["ident"] = entry.get("vor_ident")
        entry["type"] = entry.get("vor_type")
        entry["airport"] = entry.get("name")
        defaults[f"{code}_VOR"] = entry
    return defaults


DEFAULT_SAAF_BASES = _build_default_saaf_bases()
DEFAULT_VOR_STATIONS: Dict[str, Dict[str, Any]] = _build_default_vor_stations()


TERRAIN_LIBRARY: Dict[str, List[int]] = {
    "JNB": [5488, 5505, 5522, 5538, 5550, 5558, 5565, 5572, 5568, 5560, 5551, 5542, 5531, 5518, 5502],
...
class LDAFileParser:
    ...
    def parse_file(self, path: Optional[Path] = None) -> List[VORDataRecord]:
        """Parse text or compact binary LDA payloads."""
        ...
        if b"\n" in payload or payload.startswith(b"#") or payload.startswith(b"$VOR"):
            return self.parse_lines(payload.decode("utf-8", errors="ignore").splitlines())
        records: List[VORDataRecord] = []
...
class LDABinaryFileParser:
    ...
    def parse_file(self, path: Optional[Path] = None) -> List[VORDataRecord]:
        ...
        return LDAFileParser(path or self.source).parse_file()
...
class VORConnectionHandler:
    ...
    def send_command(self, cmd: str) -> str:
        """Send a command over the active link and return a text response."""
...
class DataAcquisitionThread(QThread if HAS_QT else QThread):
    ...
    def _parse_payload(self, payload: Any) -> List[VORDataRecord]:
        """Parse a raw payload into zero or more VOR data records."""
        if payload is None:
            return []
        ...
        for line in text.splitlines():
            ...
            if not stripped:
                continue
            try:
                records.append(VORDataRecord.from_sentence(stripped))
            except ValueError:
                records.extend(LDAFileParser().parse_lines([stripped]))
        return records
...
class MockVORTCPServer:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 5000, station_key: str = "JNB") -> None:
        ...
        self._thread: Optional[Thread] = None
        self._sim = VORSimEngine(station_key=station_key)

    def _handle_command(self, command: str) -> Optional[str]:
        """Return a command response when a request is recognized."""
        cmd = command.strip().upper()
        if not cmd:
            return None
        station = VOR_STATIONS.get(self.station_key, VOR_STATIONS["JNB"])
        if cmd == "STATUS":
            return f"STATUS,OK,{self.station_key},{station['vor_ident']},{station['vor_freq']:.2f}"
        if cmd == "IDENT":
            return f"IDENT,{station['vor_ident']},{morse_encode(station['vor_ident'])}"
        if cmd == "HEALTH":
            return "HEALTH,OK,SYNTHETIC,SIGNAL,NOMINAL"
        if cmd == "PING":
            return "PONG"
        return None
                    ...
                    with client:
                        ...
                        while True:
                            ...
                            try:
                                ...
                                if not command:
                                    break
                                response = self._handle_command(command.decode("utf-8", errors="ignore"))
                                if response:
...
SimulationEngine = VORSimEngine


class AircraftSimEngine:
    """En-route aircraft simulation used by the radar page."""

    def __init__(self) -> None:
        self._lock = threading.RLock()
        self._running = False
        self._last_update = time.time()
        self._reference = (-26.1337, 28.2420)
        self._aircraft: List[Dict[str, Any]] = [
            {"callsign": "SAA101", "lat": -26.30, "lon": 27.65, "alt": 31200, "spd": 430, "hdg": 075.0},
            {"callsign": "BAW054", "lat": -25.72, "lon": 28.78, "alt": 28400, "spd": 410, "hdg": 215.0},
            {"callsign": "KLM591", "lat": -26.86, "lon": 28.18, "alt": 26800, "spd": 395, "hdg": 010.0},
            {"callsign": "LNK230", "lat": -26.18, "lon": 29.05, "alt": 22600, "spd": 360, "hdg": 285.0},
            {"callsign": "SAX777", "lat": -25.48, "lon": 27.98, "alt": 19800, "spd": 320, "hdg": 165.0},
            {"callsign": "AMP901", "lat": -26.62, "lon": 27.34, "alt": 24500, "spd": 385, "hdg": 095.0},
        ]
    ...
    def _step(self, dt_seconds: float) -> None:
        ref_lat, ref_lon = self._reference
        for aircraft in self._aircraft:
            distance_nm = aircraft["spd"] * (dt_seconds / 3600.0)
            aircraft["lat"], aircraft["lon"] = destination_point(aircraft["lat"], aircraft["lon"], aircraft["hdg"], distance_nm)
            aircraft["alt"] = max(2500, aircraft["alt"] + random.randint(-120, 120))
            range_nm = haversine_nm(ref_lat, ref_lon, aircraft["lat"], aircraft["lon"])
            if range_nm > 115.0:
                aircraft["hdg"] = initial_bearing(aircraft["lat"], aircraft["lon"], ref_lat, ref_lon)
            elif random.random() < 0.15:
                aircraft["hdg"] = (aircraft["hdg"] + random.uniform(-8.0, 8.0)) % 360.0

    def get_aircraft(self) -> List[Dict[str, Any]]:
        with self._lock:
            now = time.time()
            dt_seconds = max(0.2, min(2.0, now - self._last_update))
            self._last_update = now
            if self._running:
                self._step(dt_seconds)
            return [dict(item) for item in self._aircraft]


EnRouteSimEngine = AircraftSimEngine
...
class ASRACSSimEngine:
    ...
    def get_alerts(self) -> List[IncursionAlert]:
        """Compatibility alias returning current alerts."""
        return self.alerts()

    def get_targets(self) -> List[GroundTarget]:
        """Return CVOR1-style ground targets derived from current tracks."""
        with self._lock:
            targets: List[GroundTarget] = []
            for track in self._tracks:
                history = deque([(track.x, track.y)], maxlen=120)
                targets.append(
                    GroundTarget(
                        target_id=track.callsign,
                        x=track.x,
                        y=track.y,
                        target_type=track.kind.lower(),
                        speed=track.speed,
                        heading=track.heading_deg,
                        callsign=track.callsign,
                        track_history=history,
                        route_points=[(float(x), float(y)) for x, y in track.route],
                    )
                )
            return targets
    ...
    def _update_incursion_alerts(self) -> None:
        ...
        for idx, trk_a in enumerate(self._tracks):
            for trk_b in self._tracks[idx + 1 :]:
                ...
                if sep < 10.0:
                    self._alerts.append(IncursionAlert(f"{trk_a.callsign}/{trk_b.callsign}", "CRITICAL", "Collision risk - immediate action"))
                if sep < 18.0:
...
class JNBAirportLayout:
    ...
    def __init__(self) -> None:
        self.data = copy.deepcopy(JNB_LAYOUT_DATA)

    def as_dict(self) -> Dict[str, Any]:
        """Return the detailed JNB layout dictionary."""
...
if HAS_QT:
    ...
    class CDIDisplay(QWidget):
        """CVOR1-style dot CDI compatibility widget."""
        ...
        def set_deviation(self, deviation_deg: float) -> None:
            self._deviation = float(deviation_deg)
            self.update()

        def set_data(self, deviation_deg: float, _bearing_deg: float = 0.0) -> None:
            self.set_deviation(deviation_deg)
        ...
        def paintEvent(self, _event: Any) -> None:
            ...
            for dot in range(-2, 3):
                x = int(self.width() / 2 + dot * ((right - left) / 5.0))
                painter.setBrush(QBrush(QColor(220, 220, 220)))
                painter.drawEllipse(x - 4, cy - 4, 8, 8)
            needle_x = int(self.width() / 2 + clamp(self._deviation / 2.5, -1.0, 1.0) * ((right - left) * 0.4))
            painter.setPen(QPen(QColor(255, 210, 0), 4))
            painter.drawLine(needle_x, cy - 28, needle_x, cy + 28)
            painter.setPen(QColor(220, 220, 220))
            painter.drawText(self.rect(), Qt.AlignBottom | Qt.AlignHCenter, f"LOC {self._deviation:+.2f}°")
    ...
    class RadarDisplay(QWidget):
        ...
        def __init__(self, parent: Optional[QWidget] = None) -> None:
            super().__init__(parent)
            self._center_lat = -26.1337
            self._center_lon = 28.2420
        ...
        def _advance_sweep(self) -> None:
            self._sweep = (self._sweep + 4.0) % 360.0
            self.update()

        def set_record(self, record: VORDataRecord) -> None:
            """Compatibility helper mapping a single VOR sample to one radar track."""
            lat, lon = destination_point(self._center_lat, self._center_lon, record.bearing_deg, min(record.distance_nm, self._range_nm))
            self.set_aircraft(
                [
                    {
                        "callsign": record.ident,
                        "lat": lat,
                        "lon": lon,
                        "alt": record.flags.get("altitude_ft", 0.0),
                        "spd": 0.0,
                        "hdg": record.bearing_deg,
                    }
                ]
            )

        def set_aircraft(self, aircraft_list: Sequence[Dict[str, Any]]) -> None:
            """Update radar targets and their track histories."""
            self._aircraft = [dict(item) for item in aircraft_list]
            for aircraft in self._aircraft:
                callsign = str(aircraft.get("callsign", "UNK"))
                history = self._history.setdefault(callsign, deque(maxlen=30))
                history.append((float(aircraft.get("lat", self._center_lat)), float(aircraft.get("lon", self._center_lon))))
            active = {str(item.get("callsign", "UNK")) for item in self._aircraft}
            for callsign in list(self._history.keys()):
                if callsign not in active:
                    self._history.pop(callsign, None)
            self.update()

        def _to_screen(self, lat: float, lon: float) -> Tuple[int, int]:
            """Convert lat/lon to radar pixel coordinates."""
            bearing = initial_bearing(self._center_lat, self._center_lon, lat, lon)
            distance = min(self._range_nm, haversine_nm(self._center_lat, self._center_lon, lat, lon))
            radius_ratio = distance / self._range_nm
            cx = self.width() / 2.0
            cy = self.height() / 2.0
            radius = min(self.width(), self.height()) / 2.0 - 28.0
            angle = math.radians(bearing - 90.0)
            x = int(cx + math.cos(angle) * radius * radius_ratio)
            y = int(cy + math.sin(angle) * radius * radius_ratio)
            return x, y

        def paintEvent(self, _event: Any) -> None:
            painter = QPainter(self)
            painter.setRenderHint(QPainter.Antialiasing)
            painter.fillRect(self.rect(), QColor(4, 18, 8))
            w, h = self.width(), self.height()
            cx = w // 2
            cy = h // 2
            radius = min(w, h) // 2 - 28
            painter.setPen(QPen(QColor(24, 112, 46), 1))
            for ring, miles in enumerate((30, 60, 90, 120), start=1):
                rr = int(radius * ring / 4)
                painter.drawEllipse(cx - rr, cy - rr, rr * 2, rr * 2)
                painter.drawText(cx + 6, cy - rr + 16, f"{miles}nm")
            ...
            for aircraft in self._aircraft:
                callsign = str(aircraft.get("callsign", "UNK"))
                points = [QPoint(*self._to_screen(lat, lon)) for lat, lon in self._history.get(callsign, [])]
                if len(points) > 1:
                    for idx in range(1, len(points)):
                        alpha = int(70 + (idx / len(points)) * 140)
                        painter.setPen(QPen(QColor(40, 220, 80, alpha), 2))
                        painter.drawLine(points[idx - 1], points[idx])
                x, y = self._to_screen(float(aircraft.get("lat", self._center_lat)), float(aircraft.get("lon", self._center_lon)))
                painter.setBrush(QBrush(QColor(255, 220, 0)))
                painter.setPen(QPen(QColor(255, 220, 0), 2))
                painter.drawEllipse(x - 4, y - 4, 8, 8)
                dist = haversine_nm(self._center_lat, self._center_lon, float(aircraft.get("lat", self._center_lat)), float(aircraft.get("lon", self._center_lon)))
                painter.setPen(QColor(220, 235, 220))
                painter.drawText(x + 8, y - 8, f"{callsign} {dist:.1f}nm")
    ...
    class ApproachGuidanceDisplay(QWidget):
        ...
        def __init__(self, parent: Optional[QWidget] = None) -> None:
            ...
            self._altitude_ft = self._glide.target_altitude(self._distance_nm)
            self._glide_error_ft = 0.0
            self._localizer_dots = 0.0
        ...
        def set_record(self, record: VORDataRecord) -> None:
            ...
            self._record = record
            self.set_approach_data(
                record.distance_nm,
                float(record.flags.get("altitude_ft", self._glide.target_altitude(record.distance_nm))),
                self._glide.deviation_from_path(record.distance_nm, float(record.flags.get("altitude_ft", self._glide.target_altitude(record.distance_nm)))),
                record.deviation_deg,
                self._terrain,
            )
            self.update()

        def set_approach_data(
            self,
            distance_nm: float,
            altitude_ft: float,
            glide_error_ft: float,
            localizer_dots: float,
            terrain_profile: Sequence[int],
        ) -> None:
            """Update explicit approach geometry and terrain data."""
            self._distance_nm = max(0.1, float(distance_nm))
            self._altitude_ft = float(altitude_ft)
            self._glide_error_ft = float(glide_error_ft)
            self._localizer_dots = float(localizer_dots)
            self._terrain = list(terrain_profile or self._terrain)
            self.update()
        ...
        def paintEvent(self, _event: Any) -> None:
            ...
            h = self.height()
            margin = 30
            plot = QRectF(margin, margin, (w - 2 * margin) * 0.64, h - 2 * margin)
            cdi_rect = QRectF(plot.right() + 20, margin, (w - 2 * margin) * 0.36 - 20, h - 2 * margin)
            ...
            x = plot.right() - (self._distance_nm / 15.0) * plot.width()
            y = plot.bottom() - clamp((self._altitude_ft - self._glide.runway_elevation_ft) / 3500.0, 0.0, 1.0) * plot.height()
            painter.setBrush(QBrush(QColor(0, 220, 120)))
            painter.setPen(QPen(QColor(0, 220, 120), 2))
            painter.drawEllipse(QPointF(x, y), 6, 6)
            painter.setPen(QColor(230, 230, 230))
            painter.drawText(int(plot.left()) + 10, int(plot.top()) + 20, f"Dist {self._distance_nm:.1f} NM")
            painter.drawText(int(plot.left()) + 10, int(plot.top()) + 40, f"GS {self._glide.status_label(self._distance_nm, self._altitude_ft)}")
            painter.drawText(int(plot.left()) + 10, int(plot.top()) + 60, f"Alt {self._altitude_ft:.0f} ft")
            painter.drawText(int(plot.left()) + 10, int(plot.top()) + 80, f"Err {self._glide_error_ft:+.0f} ft")

            painter.setPen(QPen(QColor(180, 180, 180), 2))
            mid_x = int(cdi_rect.center().x())
            mid_y = int(cdi_rect.center().y())
            painter.drawLine(int(cdi_rect.left()) + 20, mid_y, int(cdi_rect.right()) - 20, mid_y)
            for dot in range(-2, 3):
                dx = int(dot * cdi_rect.width() * 0.13)
                painter.setBrush(QBrush(QColor(220, 220, 220)))
                painter.drawEllipse(mid_x + dx - 4, mid_y - 4, 8, 8)
            needle_x = int(mid_x + clamp(self._localizer_dots / 2.5, -1.0, 1.0) * cdi_rect.width() * 0.28)
            painter.setPen(QPen(QColor(255, 210, 0), 4))
            painter.drawLine(needle_x, int(cdi_rect.top()) + 22, needle_x, int(cdi_rect.bottom()) - 22)
            painter.setPen(QColor(220, 220, 220))
            painter.drawText(cdi_rect, Qt.AlignBottom | Qt.AlignHCenter, f"LOC {self._localizer_dots:+.2f}")
    ...
    class ASRACSDisplay(QWidget):
        ...
        def set_targets(self, targets: Sequence[GroundTarget]) -> None:
            """Update compatibility target snapshots."""
            self._targets = list(targets)
            self.update()

        def set_alerts(self, alerts: Sequence[IncursionAlert]) -> None:
        ...
        def paintEvent(self, _event: Any) -> None:
            ...
            layout = self._jnb_layout.as_dict() if self._airport_code == "JNB" else self._layout
            ...
            for target in self._targets:
                tx, ty = map_pt((int(target.x), int(target.y)))
    ...
    class SAAFOverviewMap(QWidget):
        ...
        def paintEvent(self, _event: Any) -> None:
            ...
            for code, base in DEFAULT_SAAF_BASES.items():
    ...
    class VORAirportMonitorApp(QMainWindow):
        ...
        def __init__(self) -> None:
            ...
            super().__init__()
            self.setWindowTitle("VOR / ASRACS / SAAF Airport Monitoring System v7.0")
            self.resize(1320, 860)
            self._sim_engine = VORSimEngine()
            self._aircraft_sim = AircraftSimEngine()
            ...
            self._glide_detector = GlideSlopeDetector()
            self._surface_analyzer = SurfaceSlopeAnalyzer()
            self._config = VORAirportConfig()
            self._mock_server: Optional[MockVORTCPServer] = MockVORTCPServer()
            ...
            self._aircraft_sim.start()
            self._vor_timer = QTimer(self)
            self._vor_timer.timeout.connect(self._update_vor_synthetic)
            self._vor_timer.start(900)
            self._radar_timer = QTimer(self)
            self._radar_timer.timeout.connect(self._update_radar_display)
            self._radar_timer.start(1000)
            self._approach_timer = QTimer(self)
            self._approach_timer.timeout.connect(self._update_approach_display)
            self._approach_timer.start(500)
            self._asracs_timer = QTimer(self)
            self._asracs_timer.timeout.connect(self._update_asracs_synthetic)
            self._asracs_timer.start(250)
        ...
        def _build_saaf_db(self) -> None:
            """Load SAAF metadata into a compatibility structure."""
            self._saaf_db = copy.deepcopy(DEFAULT_SAAF_BASES)

        def _build_vor_db(self) -> None:
            """Load VOR metadata into a compatibility structure."""
            self._vor_db = copy.deepcopy(DEFAULT_VOR_STATIONS)
        ...
        def _tab_radar(self) -> None:
            ...
            if hasattr(self.aircraft_table, "horizontalHeader"):
                self.aircraft_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
            side.addWidget(self.aircraft_table, 1)
            self.aircraft_summary = QTextEdit()
            self.aircraft_summary.setReadOnly(True)
            side.addWidget(self.aircraft_summary, 1)
        ...
        def _tab_approach(self) -> None:
            ...
            layout.addWidget(self.approach, 1)
            bottom = QHBoxLayout()
            self.approach_cdi = CDIDisplay()
            bottom.addWidget(self.approach_cdi, 1)
            self.approach_info = QLabel("Approach guidance idle")
            self.approach_info.setWordWrap(True)
            bottom.addWidget(self.approach_info, 1)
        ...
        def _tab_asracs(self) -> None:
            ...
            body.addWidget(self.asracs, 2)
            self.alert_panel = ASRACSAlertPanel()
            body.addWidget(self.alert_panel, 1)
        ...
        def _tab_diagnostics(self) -> None:
            ...
            if hasattr(self.metrics_table, "horizontalHeader"):
        ...
        def _on_vor_index_changed(self, index: int) -> None:
            ...
            self._glide_detector = GlideSlopeDetector(station["elevation_ft"])
            self._surface_analyzer.set_profile(TERRAIN_LIBRARY.get(station_key, TERRAIN_LIBRARY.get("JNB", [])))
            self._config.station_key = station_key
        ...
        def _set_airport_layout(self, airport_code: str) -> None:
            ...
            self._config.airport_code = airport_code
        ...
        def _on_base_selected(self, base_code: str) -> None:
            ...
            if base_code in AIRFIELD_LAYOUTS:
                ...
                if hasattr(self, "base_map_widget"):
                    self.base_map_widget.set_base(DEFAULT_SAAF_BASES.get(base_code))
                if hasattr(self, "base_info_widget"):
                    self.base_info_widget.set_base(base_code, DEFAULT_SAAF_BASES.get(base_code))
        ...
        def _update_asracs_synthetic(self) -> None:
            ...
            self.asracs.set_targets(self._asracs_engine.get_targets())
            self.asracs.set_alerts(alerts)
            if hasattr(self, "alert_panel"):
                self.alert_panel.update_alerts(alerts)

        def _toggle_mock_server(self) -> None:
            """Start or stop the built-in mock TCP server."""
            if not getattr(self, "_mock_server_running", False):
                try:
                ...
                self._mock_server_running = True
            ...
            self._mock_server_running = False
        ...
        def _update_approach_display(self, record: Optional[VORDataRecord] = None) -> None:
            ...
            if active is not None:
                altitude_ft = float(active.flags.get("altitude_ft", self._glide_detector.target_altitude(active.distance_nm)))
                glide_error = self._glide_detector.deviation_from_path(active.distance_nm, altitude_ft)
                terrain = TERRAIN_LIBRARY.get(active.station_key, TERRAIN_LIBRARY.get("JNB", [0] * 15))
                self.approach.set_approach_data(active.distance_nm, altitude_ft, glide_error, active.deviation_deg, terrain)
                self.approach_cdi.set_deviation(active.deviation_deg)
                self.approach_info.setText(
                    f"Distance: {active.distance_nm:.2f} NM\n"
                    f"Altitude: {altitude_ft:.0f} ft\n"
                    f"Glide error: {glide_error:+.0f} ft\n"
                    f"Status: {self._glide_detector.status_label(active.distance_nm, altitude_ft)}"
                )

        def _update_radar_display(self) -> None:
            """Update radar display from the aircraft simulation."""
            aircraft_list = self._aircraft_sim.get_aircraft()
            self.radar.set_aircraft(aircraft_list)
            ...
            for row, aircraft in enumerate(aircraft_list):
                ...
                dist = haversine_nm(-26.1337, 28.2420, aircraft["lat"], aircraft["lon"])
                summary_lines.append(f"{aircraft['callsign']}: {dist:.1f} NM from JNB, FL{int(aircraft['alt'] // 100):03d}")
            self.aircraft_summary.setPlainText("\n".join(summary_lines))
        ...
        def _update_diagnostics(self) -> None:
            ...
            metrics = [
                ("Current station", latest.station_key if latest else self._config.station_key),
                ("Tracked aircraft", str(len(self._aircraft_sim.get_aircraft()))),
                ("Surface targets", str(len(self._asracs_engine.get_targets()))),
                ("Tracker keys", str(tracker_count)),
            ]
        ...
        def closeEvent(self, event: Any) -> None:
            ...
            for timer_name in ("_vor_timer", "_radar_timer", "_approach_timer", "_asracs_timer", "_diag_timer"):
    ...
    class CDIDisplay(QWidget):
        """Headless CDI compatibility placeholder."""
...
def _self_test() -> bool:
    ...
    tests.append(("vorsim-init", isinstance(VORSimEngine(), VORSimEngine)))
    ...
    tests.append(("surface-slope-init", isinstance(SurfaceSlopeAnalyzer(), SurfaceSlopeAnalyzer)))
    tests.append(("aircraft-sim-init", isinstance(AircraftSimEngine(), AircraftSimEngine)))
    ...
    tests.append(("default-saaf-count", len(DEFAULT_SAAF_BASES) == 10))
    tests.append(("saaf-squadrons", all(bool(base.get("squadrons")) for base in DEFAULT_SAAF_BASES.values())))
    ...
    sim_sentence = VORSimEngine("HECA").as_sentence()
